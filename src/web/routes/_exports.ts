...
export async function exportLibraryCsv(libraryId: string): Promise<string> {
  ...
  for (const c of copies) {
    const row = [
      ...
      c.notes ?? "",
      c.acquiredFrom ?? "",
      c.acquiredOn ? c.acquiredOn.toISOString().slice(0, 10) : "",
      c.priceCents ?? "",
      c.book.olRatingAvg ?? "",
...
export async function exportLibraryJson(libraryId: string): Promise<unknown> {
  const [library, copies, shelves, trophies, completions] = await Promise.all([
    ...
    prisma.physicalCopy.findMany({
      ...
      include: {
        ...
        photos: {
          select: { id: true, photoPath: true, label: true, position: true, createdAt: true },
  ...
  return {
    ...
    copies: copies.map((c) => ({
      ...
      acquiredFrom: c.acquiredFrom,
      acquiredOn: c.acquiredOn?.toISOString() ?? null,
